...
public class PnlItemDetailsAnzeigen {
	...
	PnlItemDetailsAnzeigen() {
		
	}
	
	public PnlItemDetailsAnzeigen(ObjectsExpenseReport objReport, String edit) {
	...
	private void initialize() {
		...
		lblCashInd = new JLabel("CASH IND:");
		...
		cmpItem = new JComboBox<Object>(category.getCategoryList());
		...
		cmpCashInd = new JComboBox<Object>(bankName.toArray());
					...
					int response = JOptionPane.showConfirmDialog(frmItemDetails, "Do you want to continue?", "Confirm",
					    ...
					    if (response == JOptionPane.NO_OPTION) {
					    	...
					    	if (edit.equals("Y")) {
						    	...
						    	fillData();
						    	updDB.updateExpenseDataBaseExcel(obj);
					    	} else {
					    		PerformDataBaseOperation dltDB = new PerformDataBaseOperation();
					    		dltDB.deleteExpenseDataBaseExcel(obj);
					    	}
					    	
					    	objReport.getFrmReport().dispose();
					    	pnlExpenseReport report = new pnlExpenseReport(objReport.getVonDate(),objReport.getBisDate(),"N");
